...
// ------------------------------------------------------------------
// Helpers: inject a known fake username for deterministic testing
// ------------------------------------------------------------------
const FAKE_USER = 'zengjie';
...
test('sk-xxx key is redacted', () => {
  const input = 'my key is sk-proj-Ax7fGhk23LmnQ9wEr4tY6uIoP1zXcVbNm8JkL';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('sk-proj-Ax7fGhk23LmnQ9wEr4tY6uIoP1zXcVbNm8JkL'),
    'full key leaked');
...
test('sk- key with dash and underscore chars', () => {
  const input = 'export KEY=sk-live_abc123DEF456_ghi789JKL012';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('sk-live_abc123DEF456_ghi789JKL012'));
  assert.ok(out.includes('sk-' + sanitize.REDACTED));
});
...
test('GEMINI_API_KEY value is removed', () => {
  const input = 'set GEMINI_API_KEY=AIzaSyDummyKeyValue1234567890abcdef';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('AIzaSyDummyKeyValue1234567890abcdef'));
  assert.ok(out.includes(sanitize.REDACTED));
});

test('AZURE_OPENAI_API_KEY value is removed', () => {
  const input = 'AZURE_OPENAI_API_KEY=abcdef1234567890abcdef1234567890';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('abcdef1234567890abcdef1234567890'));
  assert.ok(out.includes(sanitize.REDACTED));
});
...
test('Set-Cookie value is redacted', () => {
  const input = 'Set-Cookie: sessionToken=abc123def456; Path=/; HttpOnly';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('sessionToken=abc123def456'));
  assert.ok(out.includes('Set-Cookie'));
});

test('connect.sid value is redacted', () => {
  const input = 'connect.sid=s%3Aabc123def456.xyz789';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('s%3Aabc123def456.xyz789'));
  assert.ok(out.includes(sanitize.REDACTED));
});

test('sessionid assignment is redacted', () => {
  const input = 'sessionid=abc123def456ghi789jkl012';
  const out = sanitize.redactText(input, opts);
...
test('C:\\Users\\zengjie\\ path is redacted to <USER>', () => {
  const input = 'working directory: C:\\Users\\zengjie\\project\\src';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('zengjie'), 'username should not appear literally');
...
test('C:/Users/zengjie/ forward-slash path is redacted', () => {
  const input = 'path: C:/Users/zengjie/codex/sessions';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('zengjie'));
...
test('JSON double-escape Windows path is redacted', () => {
  const input = '"rawFilePath": "C:\\\\Users\\\\zengjie\\\\sessions\\\\file.jsonl"';
  const out = sanitize.redactText(input, opts);
  assert.ok(!out.includes('zengjie'));
  assert.ok(out.includes(sanitize.USER_PLACEHOLDER));
...
// ------------------------------------------------------------------
// 14. RedactPath wrapper
// ------------------------------------------------------------------
section('redactPath wrapper');

test('redactPath redacts username in path', () => {
  const input = 'C:\\Users\\zengjie\\sessions\\session.jsonl';
  const out = sanitize.redactPath(input);
  assert.ok(!out.includes('zengjie'));
  assert.ok(out.includes(sanitize.USER_PLACEHOLDER));
});
