...
//
// Contract (v0.4.1, "inventory robustness"):
...
function scan(cfg) {
  ...
  const result = {
    ...
    skippedLargeFiles: [],
    parseErrors: [],
    // v0.4.1 also exposes `errors` as a unified error list with
...
function renderMarkdown(result) {
  ...
  lines.push('');
  lines.push('> Generated by `npm run scan:sources` (CodexJournal-Lite v0.4.1).');
  ...
  lines.push('- Per the v0.4.1 contract, no IDEA log content is parsed into');
